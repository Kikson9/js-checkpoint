...
////////////////////////////////////////////
// First glimpse at callback hell
...
setTimeout(() => console.log('0 sec time'), 0); // 4
...
(async function () {
  const res = await Promise.race([
    // race among the promises and returns the first
...
// Promise.allSettled // similar to .all but never short circuits
Promise.allSettled([
...
const loadNPause = async function () {
  try {
    ...
    // load image 2
    img = await createImage2('img/img-4.jpg');
...
const loadAll = async function (imgArr) {
  try {
    const imgs = imgArr.map(async img => createImage2(img));
    console.log(imgs);
